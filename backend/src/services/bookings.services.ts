...
export const bookingService = {
	async createBooking(input: CreateBookingInput) {
		...
		const overlap = await prisma.booking.findFirst({ where: overlapWhere });
		if (overlap)
		...
		const booking = await prisma.$transaction(async (tx) => {
			const txOverlap = await tx.booking.findFirst({ where: overlapWhere });
			if (txOverlap)
			...
			return tx.booking.create({
				...
				include: {
					car: { include: { subLocation: { include: { city: true } } } },
				},
			});
		});
